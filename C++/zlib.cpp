...
void incrementalSamples(char *argv[]) {
    ...
    for (int input_size = INCREMENT_STEP_KB * KB;
        ...
        uLong ucompSize = input.length();
        uLong compSize = compressBound(ucompSize);
        void *compressed = malloc(compSize);
        void *original = malloc(input.length()*2);
        ...
        for (int j = 0; j < TOTAL_SAMPLES; ++j) {
            ...
            compress((Bytef *)compressed, &compSize, (Bytef *)input.data(), ucompSize);
            ...
            uncompress((Bytef *)original, &ucompSize, (Bytef *)compressed, compSize);
            //cout<<original[0]<<input.data()[0]<<"\t";
        ...
        for (int i = 0; i < TOTAL_SAMPLES; ++i) {
            cout << input_size << "\t" << compressedSize;
