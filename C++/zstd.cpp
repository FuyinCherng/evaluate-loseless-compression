...
void incrementalSamples(char *argv[]) {
    ...
    for (int input_size = INCREMENT_STEP_KB * KB;
        ...
        size_t const cBuffSize = 2 * input.length();
        void *const cBuff = malloc(cBuffSize);

        void *const originalBuff = malloc(input.length());
        ...
        for (int j = 0; j < TOTAL_SAMPLES; ++j) {
            ...
            compressed_size = ZSTD_compress(cBuff, cBuffSize, input.data(), input.length(), 3);
            ...
            ZSTD_decompress(originalBuff, input.length(), cBuff, compressed_size);
